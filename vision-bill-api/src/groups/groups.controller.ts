...
export class GroupsController {
  ...
  async update(@Req() req: any, @Param('id') id: string, @Body() body: any) {
    return this.groupsService.update(id, req.user.sub, body);
  }
  ...
  async addMember(@Param('id') id: string, @Req() req: any, @Body() member: any) {
    return this.groupsService.addMember(id, req.user.sub, member);
  }

  @Delete(':id/members/:index')
  async removeMember(@Param('id') id: string, @Param('index') index: number, @Req() req: any) {
    return this.groupsService.removeMember(id, req.user.sub, index);
  }
}
